...
```{r answer 5.1}
p <- ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length)) + geom_point()
ggsave(filename = "plot1.svg", plot = p, width = 6, height = 4)

png(filename = "plot2.png", width = 600, height = 400, res = 150)
p
dev.off()
```
...
png(filename = "plot_economy.png", width = 600, height = 400, res = 150)
p
dev.off()
...
```{r answerP1.5}
indel <- read.table("https://bioinformatica.uab.cat/base/documents/sgbcursos/documents/indel2019_11_5D12_17_36.txt", sep='\t', header=FALSE)

head(indel)

colnames(indel) <- c("ST", "ID", "type", "count")
head(st)

ggplot(data = indel, mapping = aes(x = type, y =count)) +
  geom_col()
```
