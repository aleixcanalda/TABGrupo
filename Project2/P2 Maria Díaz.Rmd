...
#Si, tant en size com alpha et mostra una advertència. Això és perquè en aquest tipus de gràfic. és millor visualitzar-ho amb el color o la forma, ja que es visualitza millor i realment entre les espècies no hi ha una diferència de mida, per exemple, com et fa pesar el gràfic de mida.
```
</div>
...
#En aquest plot es representa el percentatge de cotxes automàtics i manuals respecte el total en cada mida de cilindre, en comptes de posar-ho en nombre absolut.
...
#Com que la variable estava de forma numèrica, l'he hagut de canviar a un factor.
...
#Si canvies l'ordre dels elements el gràfic gira però els eixos queden al mateix lloc, per tant, les dades es vuen diferent.

#El millor gràfic és el primer, ja que d'aquesta manera et permet veure millor les mitjanes i les desviacions típiques de cada grup.
...
#És millor el viridis perquè té diferents tonalitats que les persones daltòniques podrien diferenciar encara que no vegin els diferents colors.
```
...
#Els tres son per afegir una part de text per indicar alguna cosa sobre la taula. El que canvia és la ubicació del text respecte el gràfic.
```
...
#Aquest tema és el que fa servir menys tinta però a la vegada et posa les línies per a donar una guia de la mesura, així és més fàcil d'interpretar.
```
...
```{r answer 5.1}
p <- ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length)) + geom_point()
ggsave(filename = "plot3.svg", plot = p, width = 6, height = 4)

#La diferència és que quan apropes el .png es veu menys pixelat que el .svg, per tant, té una millor qualitat.
```
...
plot <- ggplot(data = economics, mapping = aes(x = date, y = unemploy)) +
  geom_line()

plot + scale_x_date(name="Year") + scale_y_continuous(name="Unemployment") + ggtitle("Evolution of unemployment") + theme_light()

ggsave(filename = "unemployment.png", plot = plot, width = 6, height = 4)
...
```{r readSorted}
readDepth <- read.table("file:///home/samba/homes/1457766/Documentos/Arxius P1/NA18939.sorted.depth.txt", sep='\t', header=FALSE)
...
ggplot(data = readDepth, mapping = aes(x = position, y = depth, color = depth < 20)) +
  ...
  scale_x_continuous(name = "Position of chromosome 2") +
  scale_y_continuous(name = "Read depth") +
  ggtitle("Read depth along the chromosome 2 region") + 
  labs(color = "Coverage < 20") + theme_classic() +
  geom_point() 
...
```{r ST}
st <- read.table("file:///home/samba/homes/1457766/Documentos/Arxius P1/NA18939.ST.txt", sep='\t', header=FALSE)
...
```{r answerP1.5}
IDD <- read.table("file:///home/samba/homes/1457766/Documentos/Arxius P1/NA18939.indel.txt", sep='\t', header=FALSE)
...
ggplot(data = IDD, mapping = aes(x = length, y = count)) +
  scale_x_discrete(name = "Indel length") + 
  scale_y_continuous(name = "Total number") +
...
```{r pi, eval = F}
pi <- read.table("file:///home/samba/homes/1457766/Documentos/Arxius P1/out.windowed.pi", sep='\t', header=TRUE)
...
ggplot(data = pi, mapping = aes(x = BIN_START, y = PI)) +
  ...
  ggtitle("Nucleotide diversity along the chromosome 2 region") + 
  theme_light() +
...
```{r answerP1.7}
snpden <- read.table("file:///home/samba/homes/1457766/Documentos/Arxius P1/out.snpden", sep='\t', header=TRUE)

head(snpden)

ggplot(data = snpden, mapping = aes(x = BIN_START, y = VARIANTS.KB)) +
  scale_x_continuous(name="Genomic position") + 
  scale_y_continuous(name="Variants/KB") + 
  ggtitle("SNP density along the chromosome 2 region") + 
  theme_light() +
  geom_line(colour = "red4")
```
...
ggplot(data = snpden, mapping = aes(x = BIN_START, y = SNP_COUNT)) +
  scale_x_continuous(name="Genomic position") + 
  scale_y_continuous(name="SNP density") + 
  ggtitle("SNP density along the chromosome 2 region") + 
  theme_light() +
  geom_line(colour = "red4")
...
vep <- read.table("98VnIHd2eSSSkcXj.vep.txt", sep='\t', header=TRUE)

head(vep)
vepplot <- ggplot(data = vep, mapping = aes(x = Consequence)) +
  geom_bar(fill = "red4") + 
  theme_linedraw() + 
  labs(title = "VEP results", x = "Genomic element", y = "Number of SNPs") +     theme(axis.text.x = element_text(angle = 90, hjust = 1))
vepplot
ggsave(filename = "p2vep.png", plot = vepplot, width = 6, height = 10)
```
