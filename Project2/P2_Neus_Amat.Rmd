...
```{r answer1.1}
#Si, en el cas de size i alpha ens dona un warning perquè no e sla millor forma de representar les variables per tal de visualitzar-les i interpretar-les correctament. Es millor utiltizar color o shape.
...
#Ara ens fa el gràfic on a les Y en lloc del total ens mostra el percentatge en tant per u. Ens mostra el percentatge de cotxes automics i manuals respecte la mida del ciclindre.
```
...
class(mtcars$gear)
mtcars$gear_f <- factor(mtcars$gear, levels = c(3, 4, 5), labels = c("3 gears", "4 gears", "5 gears"))
ggplot(data = mtcars, mapping = aes(x = cyl_f, fill = gear_f)) +
  geom_bar()

#SI que ho hem transformat en factor.
```
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(. ~ cut ) 
#Canvia la disposició del gràfic i passa de ser horitzontal a vertical.
#El primer perquè podem veure millor la distribució normal i les mitjanes.
...
#p + scale_fill_manual(values = c("sienna1", "orange", "lightseagreen", "orangered", "red4"), name = "Quality") +
  #scale_x_continuous(name = "Weight of the diamond") + ggtitle("Diamond weight variation")
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) + scale_fill_hue ()  # try other scale_fill
#scale_fill_viridis_d() els hi anira millor perque podem distinguir millor les tonalitats, i en el hue totes les tonalitats son neutres, no van de clar a fosc.
```
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  ...
  facet_grid(cut ~ .)  +
  labs(tag = "4.2")
#Subtitle es per posar un subtitol, tag per posar un títol i caption per posar una anotació al final de la pàgina (dreta inferior).
```
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  ...
  facet_grid(cut ~ .)  +
  theme_minimal()
  # EL minimal maximitza la ink-ratio perquè fa servir menys tinta i alhora et permet interpretar el gràfic correctament.
```
...
```{r answer 5.1}
p <- ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length)) + geom_point()
ggsave(filename = "plot1.png", plot = p, width = 6, height = 4)
```
```{r answer 5.2}
p <- ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length)) + geom_point()
ggsave(filename = "plot2.svg", plot = p, width = 6, height = 4)

#Observem que es veu millor l'arxiu png, tot i que el svg s'ens obra en un programa on ens permet editar-ho.
```
...
plot4 <- ggplot(data = economics, mapping = aes(x = date, y = unemploy)) +   
geom_line()
plot4
```

```{r wrap-exercise-line-plot}
plot5 <- ggplot(data = economics, mapping = aes(x = date, y = unemploy)) +   
  labs(title="Unemployment changes during the years", x="Year", y="Unemployment") +
  theme_bw() + ggsave(filename = "plot_wrapup.png", plot = plot5, width = 6, height = 4) +
...
```{r readSorted}
readDepth <- read.table("file:///home/samba/homes/1457531/Descargas/Practica2/sorted.depth.txt", sep='\t', header=FALSE)
...
```{r answerP1.3}
plot3 <- ggplot(data = readDepth, mapping = aes(x = position, y = depth, color = depth < 20)) + scale_colour_manual(values = c("black", "red")) +
  labs(color = "Coverage < 20") + labs(title="Read depth along the chromosome 2 region", x="Position of chromosome 2",   y="Read depth") +
  theme_classic()    + geom_point()
plot3
ggsave(filename = "plot_1.png", plot = plot3 , width = 6, height = 4)
...
```{r ST}
st <- read.table("file:///home/samba/homes/1457531/Descargas/Practica2/ST1.txt", sep='\t', header=FALSE)
...
```{r answerP1.4}
plot7 <- ggplot(data = st, mapping = aes(x = type, y = count)) +
    labs(title="Substitutions", x="Type of substitution", y="Total number") +
  theme_dark() +
  geom_col(fill="red4")
plot7 
ggsave(filename = "plot_2.png", plot = plot7, width = 6, height = 4)
```
...
```{r answerP1.5}
indel <- read.table("file:///home/samba/homes/1457531/Descargas/Practica2/indel.txt", sep='\t', header=FALSE)
head(indel)
```
...
```{r answerP1.5A}
plot8 <- ggplot(data = indel, mapping = aes(x = length, y = count)) +
   labs(title="Indel length distribution", x="Total number", y="Indel legth") +
  theme_bw() + geom_col(fill="red4")

plot8
ggsave(filename = "plot_3.png", plot = plot8, width = 6, height = 4) 
```
...
```{r pi, eval = F}
pi <- read.table("file:///home/samba/homes/1457531/Descargas/Practica2/out.windowed.pi", sep='\t', header=TRUE)
...
```{r answerP1.7}
snpden <- read.table("file:///home/samba/homes/1457531/Descargas/Practica2/out.snpden", sep='\t', header=TRUE)

head(snpden)
```

```{r answerP1.6}
plot <- ggplot(data = snpden, mapping = aes(x = BIN_START, y = VARIANTS.KB)) + 
  geom_line(color="red4")
...
```{r answerP1.6}
plot <- ggplot(data = snpden, mapping = aes(x = BIN_START, y = SNP_COUNT)) + labs(title="Nucleotide diversity along the chromosome 2 region", x="Genomic position", y="Nucleotide diversity") +
  theme_bw() +
  geom_line(color="red4")

plot
ggsave(filename = "plot_6.png", plot = plot, width = 6, height = 4)
...
```{r answerP1.7A}
  vep <- read.table("file:///home/samba/homes/1457531/Descargas/Practica2/VEP/mAdUHIquj1WRoSfH.vep.txt", sep='\t', header= TRUE)
...
```{r answerP1.7A}
plot <- ggplot(data = vep, mapping = aes(x = Consequence)) +
  labs(title="VEP results", x="Genomic
  element", y="Number of SNPs") +
  theme_bw() +
