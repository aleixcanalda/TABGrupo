...
```{r answerP1.3}
p <- ggplot(data = readDepth, mapping = aes(x = position, y = depth, color = depth < 20)) +
  geom_point() + 
  scale_color_manual(values = c("black", "red")) + 
  theme_light() + 
  labs(title = "Read depth along the chromosome 2 region", fill = "Coverage < 20")
ggsave(filename = "p2graph1mydata.png", plot = p, width = 6, height = 4)
```
...
```{r answerP1.4}
pg <- ggplot(data = st, mapping = aes(x = type, y = count)) +
  geom_col(fill = "red4") + 
  theme_light() + 
  labs(title = "Substitutions", x = "Type of substitutions", y = "Total number")
ggsave(filename = "p2graph2mydata.png", plot = pg, width = 6, height = 4)
```
...
```{r answerP1.5}
indel <- read.table("indel.txt", sep='\t', header=FALSE)
colnames(indel) <- c("IDD", "ID", "length", "count")
head(indel)
pgg <- ggplot(data = indel, mapping = aes(x = length, y = count)) +
  geom_col(fill = "red4") + 
  theme_light() + 
  labs(title = "Indel length distribution", x = "Indel length", y = "Total number")
ggsave(filename = "p2graph3mydata.png", plot = pgg, width = 6, height = 4)
```
...
```{r answerP1.6}
pggp <- ggplot(data = pi, mapping = aes(x = BIN_START, y = PI)) +
...
ggsave(filename = "p2graph4mydata.png", plot = pggp, width = 6, height = 4)
```
...
```{r answerP1.7A}
snp <- read.table("out.snpden", sep='\t', header=TRUE)
head(snp)
pggpp <- ggplot(data = snp, mapping = aes(x = BIN_START, y = SNP_COUNT)) +
  geom_line(color = "red4") + 
  theme_linedraw() + 
  labs(title = "Nucleotide diversity along the chromosome 2 region", x = "Genomic position", y = "Nucleotide diversity")
ggsave(filename = "p2graph5mydata.png", plot = pggpp, width = 6, height = 4)
```
...
```{r answerP1.8}
vep <- read.table("9aWTiwpOxdGBUEDv.vep.txt", sep='\t', header=TRUE)
head(vep)
q <- ggplot(data = vep, mapping = aes(x = Consequence)) +
  geom_bar(fill = "red4") + 
  theme_linedraw() + 
  labs(title = "VEP results", x = "Genomic element", y = "Number of SNPs") +     theme(axis.text.x = element_text(angle = 90, hjust = 1))
ggsave(filename = "p2graph6mydata.png", plot = q, width = 6, height = 4)
```
